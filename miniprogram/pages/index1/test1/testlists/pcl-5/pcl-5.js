...
Page({
  ...
  tapNext() {
    if (this.data.currentChosen === -1) {
      ...
      if (this.data.nowIndex === 20) {
        //当前index为最后一个题目，准备跳转到结果分析页面
        this.calculate();
        this.setData({
          showCalculation: true,
        });
        ...
        for (var i = 0; i < this.data.Parts_Scores.length; i++) {
          partScores.push(
            String(
              this.data.Parts_Scores[i][0] +
                "：" +
                this.data.Parts_Scores[i][1] +
                "分（" +
                this.data.Parts_Scores[i][2] +
                "）；"
            )
          );
        }
